      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between px-4 pt-2 flex-wrap gap-2">
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" variant="ghost" onClick={onGoToList} className="text-xs">Careers Page</Button>
...
// // Cloudinary force-download: /upload/ ke baad fl_attachment inject karo
// // function makeDownloadUrl(url: string, filename?: string): string {
...
//   // PDF/DOC Cloudinary pe /raw/upload/ mein store hote hain
//   if (url.includes('/raw/upload/')) {
//     return url.replace('/raw/upload/', `/raw/upload/fl_attachment:${cleanName}/`);
//   }
...
//   // Non-Cloudinary URL — as-is return karo
//   return url;
// }
...
//             <Briefcase className="w-4 h-4" />
//             <span><span className="font-semibold text-slate-700">{vacancies.length}</span> vacancies</span>
//             {vacancies.filter((v) => v.open !== false).length > 0 && (
//               <>
...
//           <div className="flex gap-2">
//             {applications.length > 0 && (
//               <Button size="sm" variant="secondary" onClick={() => setView('applications')} className="text-xs gap-1.5">
//                 <Users className="w-3.5 h-3.5" /> Applications
//               </Button>
//             )}
//             <Button size="sm" variant="secondary" onClick={onGoToList} className="text-xs">Careers Page</Button>
//             <Button size="sm" onClick={() => { resetForm(); setView('editor'); }} className="text-xs gap-1.5">
//               <Plus className="w-3.5 h-3.5" /> New Vacancy
//             </Button>
//           </div>
...
//                 {['Job Title', 'Department', 'Location', 'Type', 'Status', 'Deadline', 'Actions'].map((h) => (
//                   <th key={h} className="text-left px-4 py-3.5 text-slate-500 font-mono text-xs uppercase tracking-[0.18em]">{h}</th>
//                 ))}
//               </tr>
//             </thead>
//             <tbody className="divide-y divide-slate-100">
...
//                       <Briefcase className="w-8 h-8 opacity-30" />
//                       <span className="text-sm">No vacancies yet.</span>
//                       <Button size="sm" variant="secondary" onClick={() => setView('editor')} className="mt-1 text-xs gap-1">
//                         <Plus className="w-3 h-3" /> Add first vacancy
//                       </Button>
//                     </div>
//                   </td>
...
//                   const appCount = applications.filter((a) => a.vacancySlug === v.slug).length;
//                   return (
//                     <tr key={v._id} className="align-middle hover:bg-slate-50 transition-colors">

//                       {/* Title */}
//                       <td className="px-4 py-3.5 max-w-[220px]">
//                         <div className="flex items-center gap-1.5">
//                           {v.featured && <Star className="w-3.5 h-3.5 text-yellow-400 flex-shrink-0" />}
//                           <span className="font-medium text-gray-900 truncate">{v.title}</span>
//                         </div>
//                         {appCount > 0 && (
//                           <span className="text-[11px] text-primary mt-0.5 flex items-center gap-1">
...
//                       {/* Department */}
//                       <td className="px-4 py-3.5 text-slate-500 text-xs whitespace-nowrap">{v.department}</td>
...
//                       {/* Employment type */}
//                       <td className="px-4 py-3.5">
//                         <Badge variant="outline" className="rounded-full text-[11px] text-slate-500 border-slate-200 px-2.5">
//                           {v.employmentType}
//                         </Badge>
//                       </td>
...
//                         <div className="flex items-center gap-2">
//                           <Button type="button" size="sm" variant="ghost"
//                             className="h-8 w-8 p-0 justify-center text-slate-500 hover:text-white"
//                             onClick={() => openEditor(v)} aria-label="Edit">
//                             <Pencil className="w-3.5 h-3.5" />
//                           </Button>
//                           <Button type="button" size="sm" variant="ghost"
//                             className="h-8 w-8 p-0 justify-center text-red-400 hover:text-red-300"
//                             onClick={() => handleDeleteVacancy(v)} aria-label="Delete">
//                             <Trash2 className="w-3.5 h-3.5" />
//                           </Button>
//                         </div>
...
//             </p>
//           </div>
//           <Button size="sm" variant="secondary" onClick={() => setView('list')} className="gap-1.5 text-xs">
//             <ArrowLeft className="w-3.5 h-3.5" /> Back to Vacancies
//           </Button>
//         </div>
...
//         {vacancies.length > 0 && (
//           <div className="flex items-center gap-2">
//             <span className="text-xs text-slate-500">Filter by vacancy:</span>
//             <Select value={appFilter} onValueChange={setAppFilter}>
//               <SelectTrigger className="h-8 w-[220px] text-xs border-slate-200 rounded-lg">
//                 <SelectValue placeholder="All vacancies" />
//               </SelectTrigger>
//               <SelectContent>
//                 <SelectItem value="all">All vacancies ({applications.length})</SelectItem>
...
//                   const cfg = statusConfig[app.status] || statusConfig.new;
//                   return (
//                     <tr key={app._id} className="align-middle hover:bg-slate-50 transition-colors">
...
//                           onValueChange={(val) => updateAppStatus(app._id, val as CareerApplication['status'])}
//                         >
//                           <SelectTrigger className="h-8 min-w-[140px] text-xs border-slate-200 bg-white text-gray-900 rounded-lg">
//                             <SelectValue />
//                           </SelectTrigger>
//                           <SelectContent>
//                             {Object.entries(statusConfig).map(([val, { label, cls, icon }]) => (
...
//                         <div className="flex items-center gap-2">
//                           <Button type="button" size="sm" variant="ghost"
//                             className="h-8 px-3 text-slate-500 hover:text-white text-xs"
//                             onClick={() => viewAppDetails(app)}>
//                             View
//                           </Button>
//                           {app.cvUrl && (
...
//             </div>
//             <div className="flex gap-2">
//               <Button type="button" variant="secondary" size="sm" onClick={() => setView('list')} className="gap-1.5 text-xs">
//                 <ArrowLeft className="w-3.5 h-3.5" /> All Vacancies
//               </Button>
//               {localId && (
//                 <Button type="button" variant="ghost" size="sm" onClick={handleCancel} className="text-xs">Cancel</Button>
